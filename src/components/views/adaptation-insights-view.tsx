...
import { TaskTypeId, AdaptationJobProfile } from '../../config/process-flows';
...
const DICTIONARY: DictionaryEntry[] = [
    ...
    {
        id: AdaptationJobProfile.IDENTICAL_ITEM,
    ...
    {
        id: AdaptationJobProfile.MIXED_SINGLES,
    ...
    {
        id: AdaptationJobProfile.IDENTICAL_ORDERS,
    ...
    {
        id: AdaptationJobProfile.ORDER_BASED,
    ...
    {
        id: AdaptationJobProfile.MULTI_ITEM,
    ...
    {
        id: AdaptationJobProfile.COMPLEX,
...
const PHASES = [
    {
        ...
        levers: [
            ...
            'AI Job Creation Discipline: Ensuring jobs are generated by AI, not manually.'
        ],
        metrics: [
        ...
        questions: [
            {
                ...
                logic: (stats: any, _data: ShiftRecord[], _metadata: any) => {
                    ...
                    const weights: Record<string, number> = {
                        [AdaptationJobProfile.IDENTICAL_ITEM]: 10,
                        [AdaptationJobProfile.IDENTICAL_ORDERS]: 9,
                        [AdaptationJobProfile.MIXED_SINGLES]: 8,
            ...
            {
                id: 'p1_multi_tenancy',
                text: "Location Multi-Tenancy: Are different clients' SKUs picked from the same location?",
                weight: 0.5,
                logic: (_stats: any, _data: ShiftRecord[], metadata: any) => {
                    const count = metadata.multiTenantLocs || 0;
                    const result = count > 0;
                    return {
                        answer: result ? 'Multi-Tenant' : 'Single-Tenant',
                        value: `${count} Shared Locs`,
                        isPositive: result,
                        explanation: `Identified ${count} locations storing SKUs for multiple clients. High values indicate advanced space utilization.`,
                        score: result ? 10 : 0
                    };
                }
            },
            {
                id: 'p1_ai_origin',
                text: 'AI Adoption: Are orders being generated by AI rather than manually?',
                logic: (_stats: any, data: ShiftRecord[], _metadata: any) => {
                    // Count distinct orders, flagging as AI if ANY row in the order has IsAI = true
                    ...
                    const totalOrders = orderAIMap.size;
                    if (totalOrders === 0) {
                        return {
                            answer: 'No Data',
                            value: 'No orders found',
                            isPositive: false,
                            explanation: 'No order data available to evaluate AI vs Manual origin.',
                            score: 0
                        };
                    }

                    const manualOrders = Array.from(orderAIMap.values()).filter(isAI => !isAI).length;
                    const manualRatio = manualOrders / totalOrders;
                    const manualPct = (manualRatio * 100).toFixed(2);

                    let score = 0;
                    if (manualRatio === 0) score = 10;
                    else if (manualRatio < 0.01) score = 9;
                    else if (manualRatio < 0.02) score = 8;
                    else if (manualRatio < 0.03) score = 7;
                    else if (manualRatio < 0.04) score = 6;
                    else if (manualRatio < 0.05) score = 5;
                    else if (manualRatio < 0.06) score = 4;
                    else if (manualRatio < 0.07) score = 3;
                    else if (manualRatio < 0.08) score = 2;
                    else if (manualRatio < 0.09) score = 1;
                    else score = 0;

                    return {
                        answer: manualRatio === 0 ? 'Fully AI' : `${manualPct}% Manual`,
                        value: `${manualOrders.toLocaleString()} / ${totalOrders.toLocaleString()} orders manual`,
                        isPositive: score >= 7,
                        explanation: `Manual Order Ratio: ${manualOrders} manual orders out of ${totalOrders} total distinct orders (${manualPct}%). Score reflects AI adoption discipline. Detected via IS AI? / AI Job Description column. Target: < 1% manual.`,
                        score
                    };
                }
            }
    ...
    },
    {
        ...
        questions: [
            {
                id: 'p2_density',
                text: 'Travel Sequencing: Is true physical distance logic utilized?',
                logic: (_stats: any, _data: ShiftRecord[], metadata: any, config: BufferConfig) => {
                    // Phase 2 is about distance minimization (Pathing), not pick density (Batching).
                    // True spatial routing requires the 2D Layout integration.
                    if (config.is2DLayoutUsed) {
                        return {
                            answer: 'Implemented',
                            value: '2D Layout Active',
                            isPositive: true,
                            explanation: 'Shortest-distance routing logic is active based on physical layout coordinates.',
                            score: 10
                        };
                    }
                    ...
                    return {
                        ...
                        isPositive: false,
                        explanation: 'Requires 2D Layout configuration. System is currently falling back to alphanumeric or sequential bin sorting.',
                        score: 0
                    };
                }
            }
...
export function AdaptationInsightsView({ data, config }: JobDictionaryViewProps) {
    ...
    const analysis = useMemo(() => {
        ...
        const utilTargetTypes = new Set<AdaptationJobProfile>([AdaptationJobProfile.PUT_TO_WALL, AdaptationJobProfile.MIXED_SINGLES, AdaptationJobProfile.MULTI_ITEM, AdaptationJobProfile.COMPLEX]);
        ...
        data.forEach(d => {
            ...
            // CRITICAL: Only use PICKING tasks for job classification (order/SKU/unit counts).
            // Packing and Sorting tasks are downstream duplicates that inflate unit counts.
        ...
        jobs.forEach((job, jobCode) => {
            let type: AdaptationJobProfile = AdaptationJobProfile.UNKNOWN;
            ...
            if (hasSorting) {
                ...
                type = AdaptationJobProfile.PUT_TO_WALL;
            ...
            // DEBUG: Log classification decision for each job
            debugRows.push({
                jobCode,
                wmsJobType: job.jobType,
                classifiedAs: type,
                orderCount,
                jobUniqueSkus,
                allOrdersHave1Sku,
                allOrdersHave1SkuAnd1Unit,
                areAllOrdersIdentical,
                hasSorting,
                jobVolume
            });
        });

        // Print debug table to browser console
        console.group('🔍 Adaptation Insights — Job Classification Debug');
        console.table(debugRows);
        // Summary by classified type
        const summary: Record<string, number> = {};
        debugRows.forEach(r => { summary[r.classifiedAs] = (summary[r.classifiedAs] || 0) + 1; });
        console.log('Classification Summary:', summary);
        // Show jobs where WMS type contains "single" but AI classified differently
        const mismatches = debugRows.filter(r =>
            r.wmsJobType.toLowerCase().includes('single') && r.classifiedAs !== AdaptationJobProfile.MIXED_SINGLES
        );
        if (mismatches.length > 0) {
            console.warn(`⚠️ ${mismatches.length} jobs labeled "Single" by WMS but classified differently:`);
    ...
    const scoreData = useMemo(() => {
        ...
        PHASES.forEach(phase => {
            const results = phase.questions.map(q => {
                const res = q.logic(analysis.stats, data, analysis.metadata, config);
                return { ...res, weight: (q as any).weight !== undefined ? (q as any).weight : 1.0 };
            });
            let totalWeightedScore = 0;
            let totalWeights = 0;
            results.forEach(r => {
                totalWeightedScore += (r.score || 0) * r.weight;
                totalWeights += r.weight;
            });
            const avgScore = totalWeights > 0 ? (totalWeightedScore / totalWeights) : 0;
        ...
        const weights = config.globalShiftParams?.aiMaturityWeights || { p1: 0.40, p2: 0.20, p3: 0.15, p4: 0.10, p5: 0.15 };
        const weightedScore =
            (scores['P1'].score * weights.p1) +
            (scores['P2'].score * weights.p2) +
            (scores['P3'].score * weights.p3) +
            (scores['P4'].score * weights.p4) +
            (scores['P5'].score * weights.p5);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20 p-6 lg:px-12 lg:py-8 max-w-[1600px] mx-auto w-full">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div>
                    ...
                    <h1 className="text-3xl md:text-4xl font-black text-white tracking-tight flex items-center gap-4">
                        ...
                        {/* Global Assumptions Indicators */}
                        <div className="hidden md:flex items-center gap-2 ml-2">
                            {config.is2DLayoutUsed && (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-semibold uppercase tracking-wider">
                                    <Grid className="w-3.5 h-3.5" /> 2D Layout
                                </span>
                            )}
                            {config.isEngineeredStandardsUsed && (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold uppercase tracking-wider">
                                    <Settings className="w-3.5 h-3.5" /> Engineered Stds
                                </span>
                            )}
                        </div>
                    </h1>
                ...
                <div className="flex flex-wrap gap-3">
                    <button
                        onClick={() => openHelp(<AdaptationGuide />)}
                        className="flex items-center gap-2 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 px-4 py-2 rounded-lg text-sm font-semibold transition-all"
                    >
                ...
                <div className="lg:col-span-8 flex flex-col justify-center lg:pl-4">
                    ...
                    <div className="relative w-full h-12 flex gap-1">
                        {PHASES.map((phase, i) => {
                            ...
                            return (
                                <div key={phase.id} className={`h-full flex-1 flex items-center justify-center font-bold text-xs relative group cursor-help border ${bgClass}`}>
                                    {isActive && (
                                    ...
                                    {/* Tooltip */}
                                    <div className="absolute bottom-full mb-2 hidden group-hover:block bg-slate-800 text-white text-xs p-2 rounded whitespace-nowrap border border-slate-700 z-[99999] shadow-xl">
            ...
            <div className="space-y-4">
                ...
                {/* Job Type Volume Bar Chart */}
                {(() => {
                    const JOB_COLORS: Record<string, { count: string; volume: string }> = {
                        [TaskTypeId.PUT_TO_WALL]: { count: '#f43f5e', volume: '#fda4af' },
                        [AdaptationJobProfile.IDENTICAL_ITEM]: { count: '#3b82f6', volume: '#93c5fd' },
                        [AdaptationJobProfile.MIXED_SINGLES]: { count: '#10b981', volume: '#6ee7b7' },
                        [AdaptationJobProfile.IDENTICAL_ORDERS]: { count: '#a855f7', volume: '#d8b4fe' },
                        [AdaptationJobProfile.ORDER_BASED]: { count: '#6366f1', volume: '#a5b4fc' },
                        [AdaptationJobProfile.MULTI_ITEM]: { count: '#f97316', volume: '#fdba74' },
                        [AdaptationJobProfile.COMPLEX]: { count: '#94a3b8', volume: '#cbd5e1' },
                        [AdaptationJobProfile.UNKNOWN]: { count: '#64748b', volume: '#94a3b8' },
                    };

                    const SHORT_LABELS: Record<string, string> = {
                        [TaskTypeId.PUT_TO_WALL]: 'Put-to-Wall',
                        [AdaptationJobProfile.IDENTICAL_ITEM]: 'Identical Item',
                        [AdaptationJobProfile.MIXED_SINGLES]: 'Mixed Singles',
                        [AdaptationJobProfile.IDENTICAL_ORDERS]: 'Identical Orders',
                        [AdaptationJobProfile.ORDER_BASED]: 'Order Based',
                        [AdaptationJobProfile.MULTI_ITEM]: 'Multi-Item',
                        [AdaptationJobProfile.COMPLEX]: 'Complex',
                        [AdaptationJobProfile.UNKNOWN]: 'Unknown',
                    };

                    const chartData = DICTIONARY.map(entry => {
                        const stat = analysis.stats[entry.id] || { count: 0, volume: 0 };
                        return {
                            id: entry.id,
                            name: SHORT_LABELS[entry.id] || entry.id,
                            jobs: stat.count,
                            units: stat.volume,
                            pct: totalJobs > 0 ? ((stat.count / totalJobs) * 100).toFixed(1) : '0.0',
                            fill: JOB_COLORS[entry.id]?.count || '#64748b',
                            fillVolume: JOB_COLORS[entry.id]?.volume || '#94a3b8',
                        };
                    }).filter(d => d.jobs > 0 || d.units > 0);

                    if (chartData.length === 0) return null;

                    return (
                        <div className="bg-[#0f172a] border border-slate-800 rounded-xl p-5 mb-2">
                            <div className="flex items-center justify-between mb-4">
                                <div>
                                    <p className="text-white font-semibold">Job Type Volume Distribution</p>
                                    <p className="text-slate-500 text-xs mt-0.5">Job count and unit volume breakdown across all classified job types</p>
                                </div>
                                <div className="flex items-center gap-4 text-xs text-slate-400">
                                    ...
                                    <span className="flex items-center gap-1.5">
                                        <span className="w-3 h-3 rounded-sm bg-[#6ee7b7] inline-block" />
                                        Unit Volume
                                    </span>
                                </div>
                            </div>
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        <YAxis
                                            ...
                                            tickLine={false}
                                            axisLine={false}
                                            label={{ value: 'Jobs', angle: -90, position: 'insideLeft', fill: '#64748b', fontSize: 11, dx: 8 }}
                                        />
                                        <YAxis
                                            yAxisId="right"
                                            orientation="right"
                                            ...
                                            label={{ value: 'Units', angle: 90, position: 'insideRight', fill: '#64748b', fontSize: 11, dx: -4 }}
                                        />
                                        <Tooltip
                                            cursor={{ fill: 'rgba(148,163,184,0.07)' }}
                                            contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '10px', color: '#f8fafc', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}
                                            labelStyle={{ color: '#e2e8f0', fontWeight: 700, marginBottom: 6 }}
                                            itemStyle={{ color: '#94a3b8' }}
                                            formatter={(value: number, name: string, props: any) => {
                                                if (name === 'jobs') return [value.toLocaleString(), 'Job Count'];
                                                if (name === 'units') return [value.toLocaleString(), 'Unit Volume'];
                                                return [value, name];
                                            }}
                                            labelFormatter={(label: string, payload: any[]) => {
                                                const d = payload?.[0]?.payload;
                                                return d ? `${label}  —  ${d.pct}% of jobs` : label;
                                            }}
                                        />
                                        <Bar yAxisId="left" dataKey="jobs" radius={[4, 4, 0, 0]} maxBarSize={42} animationDuration={900}>
                                            {chartData.map((entry) => (
                                                <Cell key={entry.id} fill={entry.fill} />
                                            ))}
                                        </Bar>
                                        <Bar yAxisId="right" dataKey="units" radius={[4, 4, 0, 0]} maxBarSize={42} animationDuration={1100}>
                                            {chartData.map((entry) => (
                                                <Cell key={entry.id} fill={entry.fillVolume} />
                                            ))}
                                        </Bar>
                                    </BarChart>
                                </ResponsiveContainer>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {DICTIONARY.map(entry => {
                        ...
                        const colorClass = entry.id === TaskTypeId.PUT_TO_WALL ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' :
                            entry.id === AdaptationJobProfile.IDENTICAL_ITEM ? 'text-amber-400 bg-amber-500/10 border-amber-500/20' :
