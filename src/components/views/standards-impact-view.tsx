...
import { ProcessFlowType } from '../../config/process-flows';
...
function JobBreakdownPanel({ job, config }: { job: AggregatedJob, config?: EngineeredStandardsConfig }) {
    ...
    if (isLocationBased) pickingCardId = 'picking_putwall'; // Fallback

    // Attempt explicit lookup
    const explicitPickId = `picking_${flowClass.toLowerCase().replace('-', '')}`;
    if (config.cards.find(c => c.id === explicitPickId)) {
        pickingCardId = explicitPickId;
    }
    ...
    else if (flowClass === ProcessFlowType.IIBP) packingCardId = 'packing_iibp'; // Wait, packing_iibp! Mistyped in previous code? No, line 440 says `packing_iibp`.
    else if (flowClass === ProcessFlowType.IOBP) packingCardId = 'packing_iobp'; // NEW
    else if (flowClass === ProcessFlowType.SIBP) packingCardId = 'packing_sibp';
    else if (flowClass === ProcessFlowType.SICP) packingCardId = 'packing_sicp';
    else if (flowClass === ProcessFlowType.MICP) packingCardId = 'packing_micp';
    else if (flowClass === ProcessFlowType.OBPP) packingCardId = 'packing_obpp';
