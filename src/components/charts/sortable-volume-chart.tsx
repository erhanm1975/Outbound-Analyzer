...
export function SortableVolumeChart({
    ...
    return (
        <div className="relative overflow-hidden rounded-3xl border border-slate-700/60 bg-slate-800/40 backdrop-blur-md p-8 shadow-none flex flex-col h-full group hover:-translate-y-1 transition-transform duration-300">
            ...
            {/* Header Row */}
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-3 px-2 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden md:grid shrink-0">
            ...
            <div className="space-y-4 flex-1">
                {paginatedData.map((d, i) => {
                    ...
                    return (
                        <div key={d.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center group bg-slate-800/50 rounded-xl p-2 hover:bg-slate-700/50 transition-colors animate-in fade-in slide-in-from-bottom-2 duration-300 fill-mode-both" style={{ animationDelay: `${i * 50}ms` }}>
                            <div className="md:col-span-3 text-sm font-medium text-slate-300 truncate" title={d.label}>
                                {d.label}
                            </div>

                            {/* Bar Chart Column */}
                            <div className="md:col-span-5 relative h-8 bg-slate-900/50 rounded-full overflow-visible shadow-inner">
                                <div
                                    className={cn(
                            ...
                            <div className="md:col-span-2 text-right">
                                <div className="text-sm font-bold text-slate-300">{d.metric1.toLocaleString()}</div>
                                <div className="text-[10px] text-slate-500">{labels.metric1}</div>
                            </div>
                            <div className="md:col-span-2 text-right">
                                <div className="text-sm font-bold text-slate-300">{d.metric2.toLocaleString()}</div>
                                <div className="text-[10px] text-slate-500">{labels.metric2}</div>
                            </div>
                        </div>
                    );
                })}
            ...
            <div className="mt-8 flex justify-between items-center text-xs text-slate-500 px-2 shrink-0 border-t border-slate-700/50 pt-4">
                ...
                {totalPages > 1 && (
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            disabled={page === 0}
                            className="p-1 rounded-lg hover:bg-slate-700/50 disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                        >
