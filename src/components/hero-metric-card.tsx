...
export function HeroMetricCard({ title, value, subValue, icon, trend, benchmarkValue, tooltip, className, colorClass = "from-slate-500 to-slate-700", onClick, suffix }: HeroMetricCardProps) {
    return (
        ...
        >
            ...
            {/* Glass Container */}
            <div className="relative h-full border border-white/60 dark:border-slate-800/60 bg-white/40 dark:bg-[#0b0d10]/60 backdrop-blur-xl p-8 shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] dark:shadow-none group-hover:shadow-[0_12px_40px_0_rgba(31,38,135,0.2)] dark:hover:bg-[#111418]/80 transition-all duration-300">
                ...
                <div className="flex justify-between items-start mb-6">
                    ...
                    {tooltip && (
                        <RichTooltip
                            content={tooltip}
                            trigger={
                                <div className="bg-white/50 dark:bg-slate-700/50 p-2 rounded-full cursor-help hover:bg-white/80 dark:hover:bg-slate-600/80 transition-colors">
                ...
                <div className="mt-6 pt-4 border-t border-slate-200/50 flex items-center justify-between">
                    {trend ? (
                        <div className="flex items-center gap-2">
                            {trend.value > 0 ? (
                                <div className={cn("flex items-center gap-1.5 px-2.5 py-1 rounded-full text-sm font-bold bg-white/50 dark:bg-slate-800", trend.isPositiveGood ? "text-emerald-600 bg-emerald-50/50 dark:bg-emerald-900/30 dark:text-emerald-400" : "text-rose-600 bg-rose-50/50 dark:bg-rose-900/30 dark:text-rose-400")}>
