...
export function HeroMetricCard({ title, value, subValue, icon, trend, benchmarkValue, tooltip, className, colorClass = "from-slate-500 to-slate-700", onClick, suffix }: HeroMetricCardProps) {
    return (
        ...
        >
            ...
            {/* Glass Container */}
            <div className="relative h-full border border-white/60 bg-white/40 backdrop-blur-xl p-8 shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] group-hover:shadow-[0_12px_40px_0_rgba(31,38,135,0.2)] transition-all duration-300">
                ...
                <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-slate-500 text-lg font-semibold tracking-wide">{title}</h3>
                    </div>

                    {tooltip && (
                        <div className="group/tooltip relative bg-white/50 p-2 rounded-full cursor-help hover:bg-white/80 transition-colors">
                ...
                <div className="flex flex-col">
                    <div className="flex items-baseline gap-1">
                        <span className="text-5xl font-black text-slate-800 tracking-tight leading-none bg-clip-text text-transparent bg-gradient-to-br from-slate-700 to-slate-900">
                            {value}
                        </span>
                        {suffix && <span className="text-xl font-bold text-slate-500">{suffix}</span>}
                    </div>
                    {subValue && <span className="text-sm text-slate-400 font-medium mt-1 pl-1">{subValue}</span>}
                </div>
                    ...
                    {trend ? (
                        <div className="flex items-center gap-2">
                            {trend.value > 0 ? (
                                ...
                                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-sm font-bold bg-slate-100 text-slate-400">
