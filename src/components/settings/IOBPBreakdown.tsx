...
interface IOBPBreakdownProps {
    config?: EngineeredStandardsConfig;
    onStandardsChange?: (newCards: any[]) => void;
}

export function IOBPBreakdown({ config, onStandardsChange }: IOBPBreakdownProps) {
    ...
    const renderCardTable = (cardId: string, title: string) => {
                    ...
                    <table className="w-full text-xs text-left">
                        <thead className="bg-slate-100 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400">
                            <tr>
                                ...
                                <th className="px-3 py-2 font-medium text-right">Variable Bucket</th>
                            </tr>
                        </thead>
    ...
    return (
        <div className="space-y-4 p-4 bg-slate-50 dark:bg-[#111418] rounded-lg border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">IOBP Process Breakdown (Editable)</h3>
                <RichTooltip
                    content={
                        <div className="space-y-2">
                            <p>This breakdown reflects the engineered standards applied to jobs with the "IOBP" (Identical Order Batch Pick) flow class.</p>
                            <p>You can edit the <strong>Variables</strong> (high level drivers) and <strong>Activity Standards</strong> (granular tasks) directly.</p>
                            <p><strong>Note:</strong> Editing Activity Standards will update the total calculation. Editing Variables updates the reference values used in calculations.</p>
                        </div>
                    }
                >
            ...
            <div className="space-y-8">
                {renderCardTable('picking_iobp', '1. Picking (Visit-Based + Task-Based)')}
                ...
                <div className="space-y-2">
                    ...
                    <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 text-xs text-slate-500">
                        No Sorting Activity defined for IOBP.
                    </div>
                </div>

                {renderCardTable('packing_iobp', '3. Packing (IOBP)')}
            </div>
        </div>
    );
}
