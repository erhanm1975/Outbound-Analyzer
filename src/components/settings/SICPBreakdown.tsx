...
interface SICPBreakdownProps {
    config?: EngineeredStandardsConfig;
    onStandardsChange?: (newCards: any[]) => void;
}

export function SICPBreakdown({ config, onStandardsChange }: SICPBreakdownProps) {
    ...
    return (
        <div className="space-y-4 p-4 bg-slate-50 dark:bg-[#111418] rounded-lg border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">SICP Process Breakdown (Editable)</h3>
                <RichTooltip
                    content={
                        <div className="space-y-2">
                            <p>This breakdown reflects the engineered standards applied to jobs with the "SICP" (Single Item Cluster Pick) flow class.</p>
                            <p><strong>Picking:</strong> Visit-Based (21s). Tote Assigned to SKU.</p>
                            <p><strong>Packing:</strong> Batch Overhead (Scan Tote) + Unit Variable (Make Box).</p>
                        </div>
                    }
                >
            ...
            <div className="space-y-6">
                {renderCardTable('picking_sicp', '1. Picking (Visit-Based)', 'Activity', 'Variable')}
                ...
                {renderCardTable('packing_sicp', '3. Packing (SICP Batch)', 'Activity', 'Variable')}
            </div>
        </div>
    );
}
