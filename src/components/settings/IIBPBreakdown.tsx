...
interface IIBPBreakdownProps {
    config?: EngineeredStandardsConfig;
    onStandardsChange?: (newCards: any[]) => void;
}

export function IIBPBreakdown({ config, onStandardsChange }: IIBPBreakdownProps) {
    ...
    return (
        <div className="space-y-4 p-4 bg-slate-50 dark:bg-[#111418] rounded-lg border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">IIBP Process Breakdown (Editable)</h3>
                <RichTooltip
                    content={
                        <div className="space-y-2">
                            <p>This breakdown reflects the engineered standards applied to jobs with the "IIBP" flow class.</p>
                            <p><strong>Picking:</strong> Uses Plug-and-Play "Visit-Based" Putwall logic.</p>
                            <p><strong>Packing:</strong> Uses specific IIBP Batch logic (Scan Tote + Print All Labels).</p>
                        </div>
                    }
                >
            ...
            <div className="space-y-6">
                {renderCardTable('picking_iibp', '1. Picking (IIBP)', 'Activity', 'Variable')}
                ...
                {renderCardTable('packing_iibp', '3. Packing (IIBP)', 'Activity', 'Variable')}
            </div>
        </div>
    );
}
