...
interface PutWallBreakdownProps {
    config?: EngineeredStandardsConfig;
    onStandardsChange?: (newCards: any[]) => void;
}

export function PutWallBreakdown({ config, onStandardsChange }: PutWallBreakdownProps) {
    ...
    const renderCardTable = (cardId: string, title: string, activityLabel: string, variableLabel: string) => {
        ...
        const bucketTargets: Record<string, number> = {}; // Track targets too

        card.activities.forEach(act => {
            bucketTotals[act.bucket] = (bucketTotals[act.bucket] || 0) + act.defaultSeconds;
            // Target defaults to defaultSeconds if undefined
    ...
    return (
        <div className="space-y-4 p-4 bg-slate-50 dark:bg-[#111418] rounded-lg border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Put Wall Process Breakdown (Editable)</h3>
                <RichTooltip
                    content={
                        <div className="space-y-2">
                            <p>This breakdown reflects the engineered standards applied to jobs with the "Put-Wall" flow class.</p>
                            <p><strong>Picking:</strong> Standards are applied per "Visit" (Unique Location + SKU).</p>
                            <p><strong>Sorting:</strong> Standards are applied per Unit.</p>
                            <p><strong>Packing:</strong> Standards are applied per Order (Box) and per Unit.</p>
                        </div>
                    }
                >
            ...
            <div className="space-y-6">
                {renderCardTable('picking_putwall', '1. Picking (Batch for Putwall)', 'Activity', 'Variable')}
                {renderCardTable('sorting_putwall', '2. Sorting (At Wall)', 'Activity', 'Variable')}
                {renderCardTable('packing_putwall', '3. Packing (Standard)', 'Activity', 'Variable')}
            </div>
        </div>
    );
}
