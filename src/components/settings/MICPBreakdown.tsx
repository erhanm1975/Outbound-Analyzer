...
interface MICPBreakdownProps {
    config?: EngineeredStandardsConfig;
    onStandardsChange?: (newCards: any[]) => void;
}

export function MICPBreakdown({ config, onStandardsChange }: MICPBreakdownProps) {
    ...
    return (
        <div className="space-y-4 p-4 bg-slate-50 dark:bg-[#111418] rounded-lg border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">MICP Process Breakdown (Editable)</h3>
                <RichTooltip
                    content={
                        <div className="space-y-2">
                            <p>This breakdown reflects the engineered standards applied to jobs with the "MICP" (Multi Item Cluster Pick) flow class.</p>
                            <p><strong>Picking:</strong> Visit-Based (3s) + Task Base (15s). Travel 35s.</p>
            ...
            <div className="space-y-6">
                {renderCardTable('picking_micp', '1. Picking (Visit-Based + Task-Based)', 'Activity', 'Variable')}
                ...
                {renderCardTable('packing_micp', '3. Packing (MICP)', 'Activity', 'Variable')}
            </div>
        </div>
    );
}
