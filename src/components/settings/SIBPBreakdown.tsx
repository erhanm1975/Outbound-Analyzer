...
interface SIBPBreakdownProps {
    config?: EngineeredStandardsConfig;
    onStandardsChange?: (newCards: any[]) => void;
}

export function SIBPBreakdown({ config, onStandardsChange }: SIBPBreakdownProps) {
    ...
    return (
        <div className="space-y-4 p-4 bg-slate-50 dark:bg-[#111418] rounded-lg border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">SIBP Process Breakdown (Editable)</h3>
                <RichTooltip
                    content={
                        <div className="space-y-2">
                            <p>This breakdown reflects the engineered standards applied to jobs with the "SIBP" (Single Item Batch Pick) flow class.</p>
                            <p><strong>Picking:</strong> Uses "Visit-Based" Putwall logic (21s).</p>
                            <p><strong>Packing:</strong> Uses specific SIBP Unit logic (Continuous flow from Gaylord).</p>
                        </div>
                    }
                >
            ...
            <div className="space-y-6">
                {renderCardTable('picking_sibp', '1. Picking (Visit-Based)', 'Activity', 'Variable')}
                ...
                {renderCardTable('packing_sibp', '3. Packing (SIBP - Continuous)', 'Activity', 'Variable')}
            </div>
        </div>
    );
}
