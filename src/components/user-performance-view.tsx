                ...
                <div className="bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 rounded-xl p-5 shadow-sm">
                    ...
                    <div className="space-y-3">
                        {topPerformers.map((user, idx) => (
                            <div key={user.user} className="flex items-center justify-between p-2 bg-white rounded-lg border border-indigo-50 shadow-sm relative overflow-hidden group">
                                ...
                                <div className="flex items-center gap-3 pl-2">
                                    ...
                                    <div>
                                        <div className="text-sm font-semibold text-slate-800">{user.user}</div>
                                        <div className="text-[10px] text-slate-500">{user.totalVolume} units / {user.totalShiftSpan} hrs</div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-lg font-bold text-indigo-600 leading-none">{user.uph}</div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 shadow-sm">
                    ...
                    <div className="space-y-3">
                        {lowPerformers.map((user, idx) => (
                            <div key={user.user} className="flex items-center justify-between p-2 bg-white rounded-lg border border-slate-200 shadow-sm">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs text-slate-500 font-medium">
                                        {extractInitials(user.user)}
                                    </div>
                                    <div>
                                        <div className="text-sm font-semibold text-slate-700">{user.user}</div>
                                        <div className="text-[10px] text-slate-400">{user.totalVolume} units / {user.totalShiftSpan} hrs</div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-lg font-bold text-rose-600 leading-none">{user.uph}</div>
                    ...
                    <table className="w-full text-xs text-left border-collapse">
                        <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200">
                            <tr>
                                ...
                                <th className="p-3 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('user')}>
                                ...
                                <th className="p-3 text-right cursor-pointer hover:bg-slate-100" onClick={() => handleSort('uph')}>
                                ...
                                <th className="p-3 text-right cursor-pointer hover:bg-slate-100" onClick={() => handleSort('totalVolume')}>
                                    <div className="flex items-center justify-end gap-1">
                                ...
                                <th className="p-3 text-right cursor-pointer hover:bg-slate-100" onClick={() => handleSort('totalShiftSpan')}>
                                    <div className="flex items-center justify-end gap-1">
                                ...
                                <th className="p-3 text-right cursor-pointer hover:bg-slate-100" onClick={() => handleSort('directTime')}>
                                    <div className="flex items-center justify-end gap-1">
                                ...
                                <th className="p-3 text-right cursor-pointer hover:bg-slate-100" onClick={() => handleSort('utilization')}>
                                    <div className="flex items-center justify-end gap-1">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {sortedData.map((user) => (
                                <tr key={user.user} className="hover:bg-slate-50 transition-colors group">
                                    <td className="p-3 text-center text-slate-400 font-medium">#{user.rank}</td>
                                    <td className="p-3 font-medium text-slate-700">{user.user}</td>
                                    <td className="p-3 text-right font-bold text-slate-800">{user.uph}</td>
                                    <td className="p-3 text-right text-slate-600">{(user.totalVolume ?? 0).toLocaleString()}</td>
                                    <td className="p-3 text-right text-slate-600">{user.totalShiftSpan}</td>
                                    <td className="p-3 text-right text-slate-600">{user.directTime}</td>
                                    <td className="p-3 text-right">
                                        <span className={`
                                            px-2 py-0.5 rounded-full text-[10px] font-bold
                                            ${user.utilization > 85 ? 'bg-emerald-100 text-emerald-700' :
                                                user.utilization < 50 ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600'}
                                        `}>
                                            {user.utilization}%
                                        </span>
                                    </td>
